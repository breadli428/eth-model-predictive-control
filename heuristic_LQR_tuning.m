...
for index = 1:n_samples
    ...
    if (RGB == 'g' || RGB == 'b') && power_sum <= 16 && dT_relative < dT_relative_selected
        Q = Q_idx;
...
if flag == 1
    ...
    text(power_sum_selected - 3e-2 * (max(xlim) - min(xlim)) , dT_relative_selected - 3e-2 * (max(ylim) - min(ylim)), "Q");
else
